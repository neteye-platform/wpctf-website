<template>
  <div v-if="!userMadeChoice" id="cookie-container" class="fixed top-0 bottom-0 left-0 right-0 flex pt-60 w-full h-full z-50 bg-black/80 justify-center">
    <div id="cookie" class="w-96 flex justify-center overflow-hidden">
      <div class="relative h-[fit-content] w-full text-terminal-text text-xl backdrop-blur-sm formatted-text font-terminal">
        ...
        <div class="grid grid-cols-10 text-terminal-text text-xl !leading-[1.2rem] pt-0.5 pb-1.5">
          <div class="col-span-10 left-start right-end pl-1">
            <br>
            This website uses cookies and tracking for analytics. For more information on how we use your data, please see our privacy and <a href="https://www.wuerth-phoenix.com/cookie-policy/">cookies policy</a>.
            <br><br><br>
          </div>
        </div>
        <div class="horizontal-divider"></div>
        <div class="grid grid-cols-10 text-terminal-text text-xl py-1">
          <button type="button" @click="decline" class="left-start right-end text-terminal-text text-xl h-[3.6rem] !leading-[1.2rem] col-span-5 flex-shrink-0 flex items-center  group justify-between">
            <span class="px-2">DECLINE</span>
            <span>X</span>
          </button>
          <button type="button" @click="accept" class="right-end text-terminal-text text-xl h-[3.6rem] !leading-[1.2rem] col-span-5 flex-shrink-0 flex items-center pl-4  group justify-between">
